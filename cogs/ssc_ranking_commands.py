    ...
    async def scan_scores(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                c = conn.cursor()
                c.execute("""
                    SELECT LOWER(song_name) as song_name_lower, characteristic, difficulty, level, song_name 
                    FROM ranked_maps
                """)
            ...
            for entry in all_scores:
                ...
                # Skip if we've already processed this song with a higher accuracy
                song_key = f"{song_name_lower}:{characteristic}:{difficulty}"
